...
import oscilloscup.data.Figure.Extremi;
...
/**
 * The Graphics2DPlotter is the component that paint the user's Graphics2D. The
 * paint process is not implemented in a Swing component because the user may
 * want to get a graphical reprensentation of its 2D data without showing it on
 * a user interface widget: for example the user may want to export the
 * graphical representation to an image file or event directly print it.
 * 
 * @author Luc Hogie
 */
public class Graphics2DPlotter
{
	...
	public void paint(Graphics2D graphics)
	{
		...
		graphics.fillRect(0, 0, (int) graphics.getClipBounds().getWidth(),
	...
	private void drawEverything(Graphics2D graphics)
	{
		...
		else
		{
			...
			Graphics2D spaceLegendGraphics = (Graphics2D) graphics.create(legendGraphicsX,
					legendGraphicsY, legendGraphicsW, legendGraphicsH);
			...
			int curveAndAxisLegendsGraphicsH = (int) (graphics.getClipBounds().getHeight()
					- legendGraphicsH);
			Graphics2D spaceAndDimensionLegendsGraphics = (Graphics2D) graphics.create(
	...
	private void drawAxisLegendsAndAxisAndPoints(
			Graphics2D spaceAndDimensionLegendsGraphics)
	{
		...
		if (xLegend.isVisible() && yLegend.isVisible())
		{
			...
			int curveGraphicsW = (int) (spaceAndDimensionLegendsGraphics.getClipBounds()
					.getWidth() - curveGraphicsX);
			int curveGraphicsH = (int) spaceAndDimensionLegendsGraphics.getClipBounds()
					.getHeight() - xLegend.getFont().getSize() * 2;
			Graphics2D spaceGraphics = (Graphics2D) spaceAndDimensionLegendsGraphics
					.create(curveGraphicsX, curveGraphicsY, curveGraphicsW,
							curveGraphicsH);
			...
			int xLegendGraphicsH = (int) (spaceAndDimensionLegendsGraphics.getClipBounds()
					.getHeight() - curveGraphicsH);
			Graphics2D xLegendGraphics = (Graphics2D) spaceAndDimensionLegendsGraphics
					.create(xLegendGraphicsX, xLegendGraphicsY, xLegendGraphicsW,
							xLegendGraphicsH);
			...
			Graphics2D yLegendGraphics = (Graphics2D) spaceAndDimensionLegendsGraphics
					.create(yLegendGraphicsX, yLegendGraphicsY, yLegendGraphicsW,
							yLegendGraphicsH);
