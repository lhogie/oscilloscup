...
import oscilloscup.Demo.WindowHandler;
...
public class SwingPlotter extends JComponent
{
	...
	public Dimension getPreferredSize()
	{
		...
		int h = s.getLegend().getFont().getSize()
				+ s.getXDimension().getLegend().getFont().getSize() + 250;
	...
	public void setForeground(Color fg)
	{
		throw new IllegalStateException(
				"this method shouldn't be used. Use Space.setColor() instead");
	}
	...
	{
		throw new IllegalStateException(
				"this method shouldn't be used. Use Space.setBackgroundColor() instead");
	}
	...
	public static void main(String[] args)
	{
		...
		frame.setLocation((int) (screenSize.getWidth() - side) / 2,
				(int) (screenSize.getHeight() - side) / 2);
	...
	public boolean isUpdateNeeded()
	{
		...
		else
		{
			...
			else
			{
				...
				if (image.getHeight(this) != getSize().height
						|| image.getWidth(this) != getSize().width)
				{
					return true;
				}
				else
	...
	public void setBorder(Border b)
	{
		...
		{
			// throw new
			// IllegalStateException("bordering is not allowed on a swing
			// plotter");
		}
	}
	...
	public static SwingPlotter display(Figure figure)
	{
		...
		plotter.getGraphics2DPlotter().getSpace().getXDimension().getLowerBoundAxis()
				.setVisible(false);
		plotter.getGraphics2DPlotter().getSpace().getXDimension().getUpperBoundAxis()
				.setVisible(false);
		plotter.getGraphics2DPlotter().getSpace().getYDimension().getLowerBoundAxis()
				.setVisible(false);
		plotter.getGraphics2DPlotter().getSpace().getYDimension().getUpperBoundAxis()
				.setVisible(false);
