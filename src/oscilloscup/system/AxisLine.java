...
public class AxisLine extends BoundedSpaceElement
{
	...
	protected void draw(Graphics graduationGraphics, Graphics2D graphics)
	{
		if (isVisible())
		{
			...
			setBounds(isMinimumAutomatic() ? dimension.getMin() : getMin(),
					isMaximumAutomatic() ? dimension.getMax() : getMax());
			...
			if (((Dimension) axis.getParent())
					.getOrientation() == Dimension.ORIENTATION_X)
			{
